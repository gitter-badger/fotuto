...
class HomePageTest(TestCase):
    ...
    def test_add_window_message(self):
        response = self.client.post('/windows/add/', data={'title': 'Window 1 name'})
    ...
    def test_add_window_print_message(self):
        response = self.client.post('/windows/add/', data={'name': 'Window 1 name'})
    ...
    def test_autogenerate_slug_field(self):
        window = self.save_window_form(title="Some Window Name")
        self.assertEqual(window.slug, 'some-window-name')
    ...
    def save_window_form(self, **window_data):
        """Fill :class:`~windowss.forms.WindowForm` with data specified and return instance."""
